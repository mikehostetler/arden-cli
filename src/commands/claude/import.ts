...
export function buildImportCommand(): Command {
  return new Command('import')
    ...
    .option('--claude-dir <path>', 'Custom path to Claude data directory', path.join(os.homedir(), '.claude'))
    .option('--dry-run', 'Preview files and events without sending to API', false)
...
async function processJsonlFile(filePath: string, limit: number): Promise<number> {
