...
// Mock the logger module with proper pino structure
...
describe('claude sync command', () => {
  ...
  describe('command configuration', () => {
    it('should be configured correctly', () => {
      ...
      expect(command.description()).toBe(
        'Sync Claude Code usage data from local JSONL files to Arden'
      );
  ...
  describe('file discovery', () => {
    it('should find JSONL files in project directories', async () => {
      ...
      const { findJsonlFiles } = await import('../sync');
  ...
  describe('project path extraction', () => {
    it('should extract project path from directory name', async () => {
      const { extractProjectPath } = await import('../sync');
  ...
  describe('event filtering', () => {
    it('should process assistant messages with usage data', async () => {
      const { shouldProcessEvent } = await import('../sync');
    ...
    it('should process significant user messages', async () => {
      ...
      const event = {
        ...
        message: {
          content:
            "This is a long user message that should be processed because it's over 50 characters",
        },
      };
  ...
  describe('event transformation', () => {
    it('should transform Claude event to Arden event correctly', async () => {
      const { transformToArdenEvent } = await import('../sync');
      ...
      expect(result.payload.projectPath).toBe('Users/test/project');
      expect(result.timestamp).toBe('2024-01-01T12:00:00Z');
      expect(result.payload.event.type).toBe('assistant');
      expect(result.payload.event.model).toBe('claude-3-sonnet');
      expect(result.payload.event.usage).toEqual({
        input_tokens: 100,
        output_tokens: 50,
        cache_creation_input_tokens: 10,
      });
      expect(result.payload.event.estimatedCostMicroCents).toBeGreaterThan(0);
    });
    ...
    it('should handle events without usage data', async () => {
      ...
      expect(result.payload.event.usage).toBeUndefined();
      expect(result.payload.event.estimatedCostMicroCents).toBeUndefined();
    });
    ...
    it('should calculate cost estimation correctly', async () => {
      ...
      expect(result.payload.event.estimatedCostMicroCents).toBe(1080);
    });
  });
});
