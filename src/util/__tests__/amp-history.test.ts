...
import { findAmpThreads, createAmpHistoryEvents, formatThreadSummary, AmpThread } from '../amp-history';
...
describe('amp-history', () => {
  ...
  describe('findAmpThreads', () => {
    ...
    it('should filter out non-thread entries', async () => {
      ...
      mockStat
        .mockResolvedValueOnce({ isDirectory: () => true, birthtime: new Date() } as any); // only T-123 gets stat call
